...
<template>
    <div v-show="dialog" class="tw-grid tw-h-100% tw-min-h-screen tw-w-screen tw-absolute tw-inset-0 tw--top-[5.5rem] tw-bg-black/50">
        ...
        <v-card-title style="height: 5rem; width: 40rem;">
          <p class="action-popUp-header tw-flex tw-justify-center tw-my-4">Confirm Update</p>
          <div class="popup-horizontal-line"></div>
        </v-card-title>
        <v-card-text class="d-flex align-content-center justify-center flex-wrap" style="height: 10rem; width: 40rem;"><div class="action-popUp-detail">Are you sure to update this review?</div></v-card-text>
        <v-card-actions style="height: 5rem; width: 40rem;">
          ...
          <v-btn class="tw-mx-4" style="height: 3rem; width: 7rem;" color="#1646C4" variant="flat" size="large" rounded="xl" @click="$emit('update'),$emit('toggle')">Update</v-btn>
        </v-card-actions>
      </v-card>
    </div>
</template>
