...
<template>
    ...
    <v-row no-gutters>
      ...
      <v-col cols="2" align="center">
        <div class="tw-pt-24" v-for="(book, index) in bookList.slice(1,2)">
          ...
          <v-img :src="book.file" width="150" height="250" v-if="book.file != null" cover/>
          <v-img src="/image/cover_not_available.jpg" width="150" height="250" v-if="book.file == null" cover/>
        </nuxt-link>
        </div>
      </v-col>
      <v-col cols="4" align="center">
        <div class="" v-for="(book, index) in bookList.slice(0,1)">
          <nuxt-link :to="`/book/${book.bookId}/`">
          <v-img :src="book.file" width="220" height="360" v-if="book.file != null" cover/>
        <v-img src="/image/cover_not_available.jpg" width="220" height="360" v-if="book.file == null" cover/>
      </nuxt-link>
      </div>
      </v-col>
      <v-col cols="2" align="center">
      ...
      <v-row no-gutters class="web-text-title">
        <v-col cols="2"></v-col>
        <v-col cols="2" align="center">
          <div class="tw-pt-16 tw-overflow-clip" v-for="(book, index) in bookList.slice(1,2)">
            <p>{{ book.bookName }}</p>
          </div>
        </v-col>
        <v-col cols="4" align="center">
          <div class="tw-pt-14 tw-mx-8 tw-overflow-clip" v-for="(book, index) in bookList.slice(0,1)">
            <p>{{ book.bookName }}</p>
        </div>
        </v-col>
        <v-col cols="2" align="center">
          <div class="tw-pt-16 tw-overflow-clip" v-for="(book, index) in bookList.slice(2,3)">
            <p>{{ book.bookName }}</p>
          </div>
        </v-col>
        <v-col cols="2"></v-col>
  ...
  <!-- section 2 -->
  <div class="tw-my-16">
    <div class="tw-h-min-[11rem] tw-h-max-[11rem]" v-for="(book, index) in bookList.slice(3)">
      <nuxt-link :to="`/book/${book.bookId}/`">
      <hr class="tw-my-4" />
      <v-row no-gutters>
        ...
        >
          ...
          <v-rating
                  ...
                  readonly
                ></v-rating>
            <span class="web-text-sub">({{ book.bookTotalReview }} reviews)</span>
          </div>
        </v-col>
        <v-col cols="1"></v-col>
