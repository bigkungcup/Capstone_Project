...
<template>
      ...
      <v-row>
        <v-col cols="12" align="right">
          <v-btn
            ...
            @click="book.deleteHistoryAll()"
            >Clear all</v-btn
          >
        </v-col>
      </v-row>

      <LoadingPage v-show="book.loadPage == 'load'" />
      <div v-show="book.loadPage == 'done'">
  ...
  <div v-show="book.historyList.data.content.length !== 0">
    <v-pagination
      v-model="page"
      :length="book.historyList.data.totalPages"
      :total-visible="7"
      rounded="20"
      @update:model-value="book.changeHistoryPage(page)"
    >
    </v-pagination>
  </div>
</template>
