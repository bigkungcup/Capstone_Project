                ...
                <v-row class="tw-py-2" >
                    ...
                    <v-col cols="4">
                        <div>
                            ...
                            <p class="web-text-sub tw-py-6">{{ user.userDetail.data.bio }}</p>
                        </div>
                    </v-col>

                    <v-col cols="5">
                        <div class="py-1">
                            <p class="web-text-sub tw-flex tw-place-content-end">{{ user.userDetail.data.follows }} Following {{ user.userDetail.data.followers }} Followers</p>
                        </div>
                    </v-col>
                            ...
                            <v-menu>
                                <template v-slot:activator="{ props: menu }">
                                    <v-tooltip location="top">
                                        <template v-slot:activator="{ props: tooltip }">
                                            <v-icon icon="mdi mdi-dots-vertical-circle-outline" size="x-large" v-bind="mergeProps(menu, tooltip)"></v-icon>
                                        </template>
                                        <span>More</span>
                                    </v-tooltip>
                                </template>
                                <v-list>
                                    <v-list-item :to="`/user/update_${user.userId}/`">
                                        <v-list-item-title class="web-text-detail tw-space-x-2"><v-icon icon="mdi mdi-pencil-outline"></v-icon><span>Edit this user</span></v-list-item-title>
                                    </v-list-item>
                                    <v-list-item class="hover:tw-bg-zinc-300/20 tw-cursor-pointer">
                                        <v-list-item-title class="web-text-detail">
                                            <v-list-item-title class="web-text-detail tw-space-x-2" @click="toggleUserConfirmPopup()"><v-icon icon="mdi mdi-trash-can-outline"></v-icon><span>Delete this user</span></v-list-item-title>
                                        </v-list-item-title>
                                    </v-list-item>
                                </v-list>
                            </v-menu>
