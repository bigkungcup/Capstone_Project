...
<template>
  <div class="tw-min-h-[80%]">
      ...
      <v-row no-gutters>
        <v-col cols="5">
          <v-btn size="auto" class="pa-5" color="#082266" rounded="lg">
            Result -
            {{
              user.userList.data.totalElements
                ? user.userList.data.totalElements
                : 0
            }}
          </v-btn>
    ...
    <div
      class="tw-px-36 tw-space-y-4 tw-my-4"
      v-if="user.userList.data.content.length !== 0"
    >
      <v-card class="border-md rounded-pill" color="#082266">
        <v-row no-gutters class="my-5">
          <v-col
            cols="2"
            class="user-text-title d-flex align-center justify-center"
          >
            <p>Profile</p>
          </v-col>
          <v-col
    ...
    <LoadingPage v-show="user.loadPage == 'load'" />
    <div v-show="user.loadPage == 'done'">
