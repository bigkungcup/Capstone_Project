...
-- -----------------------------------------------------
-- Table `bannarug`.`FollowerStatus`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bannarug`.`FollowerStatus` ;

CREATE TABLE IF NOT EXISTS `bannarug`.`FollowerStatus` (
  `followerStatusId` INT NOT NULL,
  `fsu_userFollowerId` INT NOT NULL,
  `fsu_userId` INT NOT NULL,
  `followerStatus` INT NULL,
  `followerStatusCreateDateTime` DATETIME NULL,
  `followerStatusUpdateDateTime` DATETIME NULL,
  INDEX `fk_Follower_has_User_User1_idx` (`fsu_userId` ASC) VISIBLE,
  INDEX `fk_Follower_has_User_Follower1_idx` (`fsu_userFollowerId` ASC) VISIBLE,
  PRIMARY KEY (`followerStatusId`),
  CONSTRAINT `fk_Follower_has_User_Follower1`
    FOREIGN KEY (`fsu_userFollowerId`)
    REFERENCES `bannarug`.`Follower` (`userFollowerId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Follower_has_User_User1`
    FOREIGN KEY (`fsu_userId`)
    REFERENCES `bannarug`.`User` (`userId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
