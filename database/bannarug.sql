...
-- -----------------------------------------------------
-- Table `bannarug`.`CheckLikeReview`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bannarug`.`CheckLikeReview` ;

CREATE TABLE IF NOT EXISTS `bannarug`.`CheckLikeReview` (
  `checkLikeReviewId` INT NOT NULL AUTO_INCREMENT,
  `clr_reviewId` INT NOT NULL,
  `clu_userId` INT NOT NULL,
  `likeStatus` INT NULL,
  `checkLikeReviewCreateDateTime` DATETIME NULL,
  `checkLikeReviewUpdateDateTime` DATETIME NULL,
  PRIMARY KEY (`checkLikeReviewId`, `clr_reviewId`, `clu_userId`),
  INDEX `fk_Review_has_User_User1_idx` (`clu_userId` ASC) VISIBLE,
  INDEX `fk_Review_has_User_Review1_idx` (`clr_reviewId` ASC) VISIBLE,
  CONSTRAINT `fk_Review_has_User_Review1`
    FOREIGN KEY (`clr_reviewId`)
    REFERENCES `bannarug`.`Review` (`reviewId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Review_has_User_User1`
    FOREIGN KEY (`clu_userId`)
    REFERENCES `bannarug`.`User` (`userId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
